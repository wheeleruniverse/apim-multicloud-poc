...
if command -v gh &> /dev/null; then
    ...
    if [ "$CONFIGURE_GH" = "y" ] || [ "$CONFIGURE_GH" = "Y" ]; then
        ...
        echo -e "${BLUE}------------------------------------------------------------${NC}"
        echo ""
        echo "1. Ensure Azure secrets and variables are configured (if not, run setup-azure-identity.sh)"
        echo ""
        ...
        echo "   - Environment: $ENVIRONMENT"
        echo ""
        echo "4. After Azure deployment succeeds, generate the APIM gateway token:"
        echo ""
        ...
        echo "     --expiry 2025-12-31T23:59:59Z \\"
        echo "     --query value -o tsv"
        echo ""
        echo "5. Add the gateway token to GitHub using gh CLI:"
        echo ""
        ...
        echo ""
        echo "4. Run the Azure deployment workflow first to create APIM"
        echo ""
        echo "5. After Azure deployment, generate the APIM gateway token:"
        echo "   az apim gateway token create \\"
        echo "     --resource-group ${PROJECT_NAME}-${ENVIRONMENT}-apim-rg \\"
        echo "     --service-name ${PROJECT_NAME}-${ENVIRONMENT}-apim \\"
        echo "     --gateway-id aws-self-hosted-gateway \\"
        ...
        echo "     --query value -o tsv"
        echo ""
        echo "6. Add the gateway token as a GitHub secret:"
        echo "   Secret Name: TF_VAR_apim_gateway_token"
        echo "   Value: <token from step 5>"
        echo ""
        echo "7. Run the AWS deployment workflow"
        echo ""
    fi
else
